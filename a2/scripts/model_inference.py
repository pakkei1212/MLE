...
def main(args):
    ...
    # ---------- Save Predictions Locally (CSV + Parquet optional) ----------
    out_dir = os.path.join("datamart", "gold", "model_predictions")
    ...
    print(f"[INFER] Saved predictions to {csv_path} and {parquet_path}")
    ...
    with mlflow.start_run(run_name=f"infer_{args.snapshotdate.replace('-', '_')}") as run:
        ...
        # Log files to MLflow as artifacts
        mlflow.log_artifact(csv_path, artifact_path="predictions")
        mlflow.log_artifact(parquet_path, artifact_path="predictions")

        print(f"[INFER] Logged {len(out_df)} rows to MLflow run_id={run.info.run_id}")
        print(f"[INFER] artifact_uri: {mlflow.get_artifact_uri('predictions')}")


if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Score Gold pretrain features and log predictions to MLflow and CSV.")
