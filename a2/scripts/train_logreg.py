...
def main(args):
    ...
    param_dist = {
        ...
        "clf__solver": ["lbfgs", "liblinear", "saga"],     # try a few
        "clf__penalty": ["l2"],
    ...
    write_model_registry_row(
        ...
        windows["train_test_start_date"], windows["train_test_end_date"],
        windows["oot_start_date"], windows["oot_end_date"],
        auc_train, auc_test, auc_oot,
        promoted_flag, promoted_at_iso
    )
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Train Logistic Regression on Gold features + labels (Parquet registry)")
    ...
    ap.add_argument("--n-iter", type=int, default=20)  # faster than tree models
